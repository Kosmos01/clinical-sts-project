...
def main():
    ...
    '''
    CUIs, preferredText, sentences, posTags, posWords  = ctakesAttributes.grabUMLSAttributes()
    ...
    '''
    print('working on bio embeddings')
    bioEmbeddings = Berts.loadEmbeddings(whichBert='bio')
    #print('working on clinical embeddings')
    clinicalEmbeddings = Berts.loadEmbeddings(whichBert='clinical')
    print('working on discharged embeddings')
    dischargeEmbeddings = Berts.loadEmbeddings(whichBert='discharge')
    print('working on masked embeddings')
    maskedEmbeddings = Berts.loadEmbeddings(whichBert='masked')
    '''

    '''
    print('calculating attributes')
    TestSet.calculateAttributes(bio=bioEmbeddings,clinical=clinicalEmbeddings,discharged=dischargeEmbeddings,masked=maskedEmbeddings)
    '''
    ...
    ################################################################
    ######## Dev Section - Feature selection via default parameters
    ################################################################
    ...
    # run rf
    # gatherPearsons.append(trainDev(linear_model.Ridge(random_state=42),train,test,params))
    ...
    # run knn
    # gatherPearsons.append(trainDev(linear_model.BayesianRidge(),train,test,params))
    ...
    # run Gaussian NB
    # gatherPearsons.append(trainDev(ensemble.AdaBoostRegressor(random_state=42),train,test,params))
    ...
    # gatherPearsons.append(trainDev(ensemble.GradientBoostingRegressor(random_state=42),train,test,params))
    ...
    ################################################################
    ######## Dev Section - tuning parameter WAS FOR SUMMER RESEARCH PROGRAM
    ################################################################
    ...
    # linear
    predictionsTune = trainDev(linear_model.LinearRegression(), train, test, parameters)
    ...
    clf = RandomizedSearchCV(estimator=linear_model.Ridge(random_state=42),param_distributions=ridgeParams,
                       n_jobs=-1,n_iter=100,cv=5,verbose=0)

    predictionsTune += trainDev(clf,train,test,parameters)
    print('ridge best params: ' + str(clf.best_params_))
    ...
    clf = RandomizedSearchCV(estimator=linear_model.BayesianRidge(), param_distributions=bayesianParams,
                              n_jobs=-1, n_iter=100, cv=5, verbose=0)

    predictionsTune += trainDev(clf,train,test,parameters)
    print('bayesian best params: ' + str(clf.best_params_))
    ...
    clf = RandomizedSearchCV(estimator=ensemble.AdaBoostRegressor(random_state=42), param_distributions=adaParams,
                              n_jobs=-1, n_iter=100, cv=5, verbose=0)

    predictionsTune += trainDev(clf,train,test,parameters)
    print('ada best params: ' + str(clf.best_params_))
    ...
    clf = RandomizedSearchCV(estimator=ensemble.GradientBoostingRegressor(random_state=42), param_distributions=gradParams,
                              n_jobs=-1, n_iter=100, cv=5, verbose=0)

    predictionsTune += trainDev(clf, train, test, parameters)
    print('ada best params: ' + str(clf.best_params_))

    predictionsTune = predictionsTune / 5

    print(predictionsTune)

    computePearson(test['score'], predictionsTune)
    '''
